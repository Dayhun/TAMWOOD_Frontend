...
let loadTable = () => {
    $.getJSON(
        "http://127.0.0.1:5500/06-Ajax/Class-01/jsonInvoiceSystem/data/products.json",
        (result) => {
...
$("form")
    ...
    .change((e) => {
        ...
        let loadJson = () => {
            $.getJSON(
                "http://127.0.0.1:5500/06-Ajax/Class-01/jsonInvoiceSystem/data/customers.json",
                (data) => {
                    data.forEach((dt) => {
                        if (dt.id == parseInt($(e.target).val())) {
                            $(e.target).next().val(dt.first_name);
                            user = new User(dt.id, dt.first_name);
                        }
                    });
                    if (user == null) {
                        alert("No data exist");
...
let tablePoper = () => {
    ...
    for (let pro of productList.values()) {
        ...
        sum += parseFloat(pro.total());
    }
    $("#invoice tfoot tr").children().eq(1).text(sum);
};
